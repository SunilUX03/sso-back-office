...
function renderOverview() {
  ...
  Store.visibleDepartments().forEach((dept) => {
    ...
    deptTotal += Store.deptOffices(dept, Store.generalSubDept()).length;
...
// ---- A department's OWN jurisdiction offices only (its "General /
// Department-Level" bucket) — deliberately NOT summed with its
// sub-departments', so the chips on its card only ever describe offices
// that belong to the department itself, e.g. a Secretariat office. ----
function deptOwnLevelCounts(deptName) {
  return Store.deptLevelCounts(deptName, Store.generalSubDept());
}
// How many of a department's real sub-departments have at least one
// jurisdiction office of their own.
function subDeptsConfiguredCount(deptName) {
  return Store.allSubDepartments(deptName).filter((sub) => Store.deptOffices(deptName, sub).length > 0).length;
}
// A department counts as Configured if EITHER it has its own
// department-level jurisdictions, OR at least one of its sub-departments
...
// did NOT — i.e. exactly the case where an admin knows the sub-department
// but not which department it sits under. Skipped when the parent already
// matched, since its card already leads there.
function subDeptHitResults() {
  ...
  Store.visibleDepartments().forEach((dept) => {
    ...
    Store.allSubDepartments(dept).forEach((sub) => {
      ...
      const counts = Store.deptLevelCounts(dept, sub);
  ...
  depts.forEach((name) => {
    ...
    deptList.appendChild(
      el(
        ...
        `<a class="ux4g-jm-dept-link" href="jurisdiction-subdept.html?dept=${Store.deptSlug(name)}" aria-label="Open ${escapeHtml(name)}">
           <span class="ux4g-jm-dept-badge"><span class="ux4g-icon-outlined">domain</span></span>
    ...
    subHits.forEach(({ dept, sub, counts }) => {
      deptList.appendChild(
        el(
          "article",
          "ux4g-card ux4g-card-outline ux4g-jm-dept-card",
          `<a class="ux4g-jm-dept-link" href="agency.html?dept=${Store.deptSlug(dept)}&sub=${Store.subDeptSlug(sub)}" aria-label="Open ${escapeHtml(sub)}">
             <span class="ux4g-jm-dept-badge"><span class="ux4g-icon-outlined">account_tree</span></span>
             <span class="ux4g-jm-dept-body">
...
function openSubDeptStatus(dept) {
  window.location.href = `jurisdiction-subdept.html?dept=${Store.deptSlug(dept)}&filter=configured#subDepartments`;
}
