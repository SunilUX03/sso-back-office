// ============================================================
// TN SSO — Jurisdiction Management: Sub-Department picker.
...
function renderGeneral() {
  ...
  const counts = Store.deptLevelCounts(deptName, sub);
  ...
  generalList.appendChild(
    el(
      ...
      `<a class="ux4g-jm-dept-link" href="agency.html?dept=${Store.deptSlug(deptName)}&sub=${Store.subDeptSlug(sub)}" aria-label="Open ${escapeHtml(label)}">
    ...
    .forEach((name) => {
      ...
      subDeptList.appendChild(
        el(
          "article",
          "ux4g-card ux4g-card-outline ux4g-jm-dept-card",
          `<a class="ux4g-jm-dept-link" href="agency.html?dept=${Store.deptSlug(deptName)}&sub=${Store.subDeptSlug(name)}" aria-label="Open ${escapeHtml(name)}">
